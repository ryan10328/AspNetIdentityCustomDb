...
using AspIdentityCustomDb.Models;
...
namespace AspIdentityCustomDb
{
    ...
    public class ApplicationUserManager : UserManager<User>
    {
        ...
        public static ApplicationUserManager Create(IdentityFactoryOptions<ApplicationUserManager> options, IOwinContext context)
        {
            ...
            // 您可以撰寫專屬提供者，並將它外掛到這裡。
            manager.RegisterTwoFactorProvider("電話代碼", new PhoneNumberTokenProvider<User>
            {
                MessageFormat = "您的安全碼為 {0}"
            });
            manager.RegisterTwoFactorProvider("電子郵件代碼", new EmailTokenProvider<User>
            {
                Subject = "安全碼",
                BodyFormat = "您的安全碼為 {0}"
            });
